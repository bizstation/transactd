...
public:
	...
	inline void setJoinRowMap(const std::vector< std::vector<int> >* v){m_joinRowMap = v;/*m_joinMapSize = size;*/}
...
// class recordset
// ---------------------------------------------------------------------------
recordset::recordset():m_fds(fielddefs::create(), &fielddefs::destroy)
...
void recordset::registerMemoryBlock(unsigned char* ptr, size_t size, size_t recordLen
...
recordset::iterator recordset::begin(){return m_recordset.begin();}

recordset::iterator recordset::end(){return m_recordset.end();}

recordset::iterator recordset::erase(size_t index){return m_recordset.erase(m_recordset.begin()+index);}

recordset::iterator recordset::erase(const iterator& it){return m_recordset.erase(it);}

void recordset::push_back(row_ptr r){m_recordset.push_back(r);};
...
recordset::key_type recordset::resolvKeyValue(const std::_tstring& name, bool noexception)
{
	int index = m_fds->indexByName(name);
	if (index != -1)
		return index;
