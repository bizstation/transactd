...
void showResult(const std::vector<transactionSec>& results
		, int total, int totalTransactionTime,int totalTransactions, bool stressMode)
{
	...
	if (!stressMode)
	{
		...
		std::tcout << _T("avarage transaction time (sec) ") << totalTransactionTime/1000000.0f/totalTransactions << std::endl;
...
void makeMysqlParam(bzs::test::worker::mysql::connectParam& param, const _TCHAR* host)
{
	#ifdef _UNICODE
		char hostname[256];
...
double executeWorkers(const commandLineParam& cmd
	...
	for (int k=0;k<cmd.avg_count;++k)
	{
		...
		for (int i=0;i<wn;++i)
		{
			...
			if (cmd.funcNumber < 10)
				w.reset(new transactd::worker(id, cmd.loopCount, cmd.funcNumber, param, sync));
			else
				w.reset(new bzs::test::worker::mysql::worker(id, cmd.loopCount, cmd.funcNumber, paramMysql, sync));
			workers.push_back(w);
...
int execute(const commandLineParam& cmd
		,const connectParams param
		,const bzs::test::worker::mysql::connectParam& paramMysql
		...
		,double& totalTransactionTime)
{
	...
	for (int wn=cmd.workerNumStep;wn<=cmd.maxWorkerNum; wn+=cmd.workerNumStep)
	{
		double t = executeWorkers(cmd, param, paramMysql, wn);
		transactionSec ts;
		if (cmd.stressMode)
		{
			if (t) ts.value =(__int64)( t/BENCH_TIMER_SECONDS);
		}
		else
		{
			totalTransactionTime += t;
			totalTransactions += wn * cmd.loopCount * cmd.avg_count;
			if (t) ts.value = (__int64)((double)1000000.0f*wn*wn*cmd.avg_count*cmd.loopCount/t);
		}
		ts.workes = wn;
...
int _tmain(int argc, _TCHAR* argv[])
{
	try
	{
		...
		bzs::test::worker::mysql::connectParam paramMysql(argv[1], _T("querytest"), _T("root"));
		...
		if (tableFixer(param))
		{
			...
			bm.start();
			int totalTransactions = execute(cmd, param, paramMysql, results, totalTransactionTime);
			printf("\n");
