...
int readConfig(conf_parmas& pmc, const cmd_parmas& pm)
{
	...
	cnf.add_options()
		("lang",value<string>(&pmc.lang),"lang")
		("files",value<int>(&pmc.files),"files")
		("file1", value<string>(&pmc.file[0]),"file1")
		("file2", value<string>(&pmc.file[1]),"file2")
		("name_space", value<string>(&pmc.name_space),"name_space")
		("name_space_map", value<string>(&pmc.name_space_map),"name_space_map")
		("saveDir", value<string>(&pmc.saveDir),"saveDir")
		("setPrefix", value<string>(&pmc.setPrefix),"setPrefix")
		("getPrefix", value<string>(&pmc.getPrefix),"getPrefix")
		("externWord", value<string>(&pmc.externWord),"externWord")
		("fieldRenameList", value<string>(&pmc.fieldRenameList),"fieldRenameList");
...
int _tmain(int argc, char* argv[])
{
	...
	opt.add_options()
		("database_uri,d",value<string>(&pm.uri),"database uri ex:tdap://hostname/dbname?dbfile=trnasctd_schema")
	...
	try
	{
		...
		notify(values);
		if (!values.count("database_uri")
				|| !values.count("table_name"))
		{
			cout << opt << endl;
			return 1;
		}
		...
		if (values.count("conf_name"))
		{
			...
			{
				int n = db->dbDef()->tableNumByName(toTString(pm.tableName).c_str());
				if (n <= 0)
				{
