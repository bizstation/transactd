...
void cppSrcGen::replace(string& source, const char* convTgtStr, const string& replaceStr)
{
	size_t pos = 0;
...
int cppSrcGen::typeNum(int type)
{
	...
	if (t == ft_autoIncUnsigned)
		...
		t = 1;
	else if ((t == ft_note) || (t == ft_zstring)
		|| (t == ft_wzstring) || (t == ft_myvarchar) || (t == ft_mywvarchar)
		|| (t == ft_myvarbinary) || (t == ft_mywvarbinary) || (t == ft_mytext)
		|| (t == ft_guid) || (t == ft_lstring))
		t = 0;
	else if (t == ft_float)
		t = 2;
...
string cppSrcGen::makeDataMembaFuncString()
{
	...
	for (int i=0;i<m_tabledef->fieldCount;i++)
	{
		...
		{
			tmp = "const _TCHAR* " + m_pm.dataClassName + "::"+ membaNameGet(name) + "()const" LF "{" LF "\treturn m_impl->"+ name +".c_str();" LF "}" LF;
			tmp2 = "void " + m_pm.dataClassName + "::" + membaNameSet(name) + "(const _TCHAR* v)" LF "{" LF "\tm_impl->"+ name + " = v;" LF "}" LF;
		}else if ((t == ft_time) || (t == ft_date))
		{
			tmp = string("const ") + ts
					+ "& " + m_pm.dataClassName + "::" + membaNameGet(name)
					+ "() const" LF "{" LF "\treturn m_impl->" + name + ";" LF "}" LF;
			tmp2 = "void " + m_pm.dataClassName + "::" + membaNameSet(name) +  "(int v)" LF "{" LF "\tm_impl->"
					+ name + ".i = v;" LF "}" LF;
		}else if (t == ft_datetime)
		{
			tmp = string("const ") + ts
			...
			tmp2 = "void " + m_pm.dataClassName + "::" + membaNameSet(name)
					+  "(__int64 v)" LF "{" LF "\tm_impl->" + name + ".i64 = v;" LF "}" LF;
		...
		{
			tmp = string() + ts
					+ string(" ") + m_pm.dataClassName + "::" + membaNameGet(name) + "() const" LF "{" LF "\treturn m_impl->" + name + ";" LF "}" LF;
			tmp2 = "void " + m_pm.dataClassName + "::" + membaNameSet(name) + "("
					+ ts + " v)" LF "{" LF "\tm_impl->"+ name + " = v;" LF "}" LF;
		}
...
string cppSrcGen::makeDataMembaFuncDecString()
{
	...
	for (int i=0;i<m_tabledef->fieldCount;i++)
	{
		...
		else if (t == ft_lvar)
		{
			...
			tmp = ts + " " + membaNameGet(name) + "(unsigned int& size) const;" LF;
			tmp2 = "void " + membaNameSet(name) +  "(void* v, unsigned int size);" LF;
		}
		else
		{
...
{
	string ret;
	if ((m_tabledef->primaryKeyNum >= 0)  && (m_tabledef->primaryKeyNum < m_tabledef->keyCount))
	{
...
string cppSrcGen::makeMapAutoIncString()
{
	...
	for (int i=0;i<m_tabledef->fieldCount;i++)
	{
		...
		{
			string name =  membaName(fd);
			retVal = retVal + "\tm."+ membaNameSet(name)
			  + "(fds[m_fdi."+ name + "]." + intReadMenbaName(fd) + ");" LF;
		}
	}
	removeEndchar(retVal);
	return retVal;
...
{
	const vector<string>& list = data ?  m_pm.name_spaces(): m_pm.name_spaces_map();
...
bool cppSrcGen::doMake(const string templeHeader, const string templeCpp, bool data)
{
	//header
	{
		string hd;
		...
		if (data)
		{
			replace(hd ,"%dataClassMembaFuncDec%", makeDataMembaFuncDecString());
			replace(hd ,"%nameSpaceBegin%", makeNameSpaceBegin(m_pm.name_spaces()));
		...
		else
		{
			...
			replace(hd ,"%nameSpaceMapBegin%", makeNameSpaceBegin(m_pm.name_spaces_map()));
			replace(hd ,"%nameSpaceMapEnd%", makeNameSpaceEnd(m_pm.name_spaces_map()));
		}
	...
	//cpp
	{
		...
		if (data)
		{
			...
			replace(cpp ,"%nameSpaceBegin%", makeNameSpaceBegin(m_pm.name_spaces()));
			replace(cpp ,"%nameSpaceEnd%", makeNameSpaceEnd(m_pm.name_spaces()));
		}
		else
		{
			...
			replace(cpp ,"%nameSpaceMapBegin%", makeNameSpaceBegin(m_pm.name_spaces_map()));
			replace(cpp ,"%nameSpaceMapEnd%", makeNameSpaceEnd(m_pm.name_spaces_map()));
		}
