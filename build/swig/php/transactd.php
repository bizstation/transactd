...
abstract class transactd {
	...
	const MBC_CHARSETNAME = MBC_CHARSETNAME;

	const UTF8_CHARSETNAME = UTF8_CHARSETNAME;

	static function u8mbcvt_set($u8mbcvt) {
		u8mbcvt_set($u8mbcvt);
	}

	static function u8mbcvt_get() {
		return u8mbcvt_get();
	}

	static function mbu8cvt_set($mbu8cvt) {
		mbu8cvt_set($mbu8cvt);
	}

	static function mbu8cvt_get() {
		return mbu8cvt_get();
	}

	static function mbcscvt_set($mbcscvt) {
		mbcscvt_set($mbcscvt);
	}

	static function mbcscvt_get() {
		return mbcscvt_get();
	}

	static function wchrcvt_set($wchrcvt) {
		wchrcvt_set($wchrcvt);
	}

	static function wchrcvt_get() {
		return wchrcvt_get();
	}

	static function u8wccvt_set($u8wccvt) {
		u8wccvt_set($u8wccvt);
	}

	static function u8wccvt_get() {
		return u8wccvt_get();
	}

	static function wcu8cvt_set($wcu8cvt) {
		wcu8cvt_set($wcu8cvt);
	}

	static function wcu8cvt_get() {
		return wcu8cvt_get();
	}

	static function WideCharToMultiByte($codepage,$dwFlags,$lpWideCharStr,$cchWideChar,$lpMultiByteStr,$cchMultiByte,$lpDefaultChar,$lpUsedDefaultChar) {
	...
	static function MultiByteToWideChar($codepage,$dwFlags,$lpMultiByteStr,$cchMultiByte,$lpWideCharStr,$cchWideChar) {
		return MultiByteToWideChar($codepage,$dwFlags,$lpMultiByteStr,$cchMultiByte,$lpWideCharStr,$cchWideChar);
	}

	static function u8tombc($u8,$u8size,$mbc,$mbcsize) {
		return u8tombc($u8,$u8size,$mbc,$mbcsize);
	}

	static function mbctou8($mbc,$mbcsize,$u8,$u8size) {
		return mbctou8($mbc,$mbcsize,$u8,$u8size);
	}

	const MB_PRECOMPOSED = MB_PRECOMPOSED;

	const WC_COMPOSITECHECK = WC_COMPOSITECHECK;
	...
	static function host($uri,$buf) {
		return host($uri,$buf);
	}

	static function dbname($uri,$buf) {
		return dbname($uri,$buf);
	}

	static function schemaTable($uri,$buf) {
		return schemaTable($uri,$buf);
	}
