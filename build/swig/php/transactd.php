...
abstract class transactd {
	...
	static function copy_memoryRecord_p_p($value) {
		return copy_memoryRecord_p_p($value);
	}

	static function delete_memoryRecord_p_p($obj) {
		delete_memoryRecord_p_p($obj);
	}

	static function memoryRecord_p_p_assign($obj,$value) {
		memoryRecord_p_p_assign($obj,$value);
	}

	static function memoryRecord_p_p_value($obj) {
		$r=memoryRecord_p_p_value($obj);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
			return new Record($r);
...
abstract class fieldsBase {
	...
	function indexByName($name) {
		return fieldsBase_indexByName($this->_cPtr,$name);
	}

	function fieldDefs() {
		$r=fieldsBase_fieldDefs($this->_cPtr);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
...
class Record extends fieldsBase implements \ArrayAccess, \Countable, \IteratorAggregate {
	...
	function __set($var,$value) {
		if ($var === 'thisown') return swig_transactd_alter_newobject($this->_cPtr,$value);
		fieldsBase::__set($var,$value);
	}

	function __isset($var) {
		if ($var === 'thisown') return true;
		return fieldsBase::__isset($var);
	}

	function __get($var) {
		if ($var === 'thisown') return swig_transactd_get_newobject($this->_cPtr);
		return fieldsBase::__get($var);
	}
	function __construct($h) {
		$this->_cPtr=$h;
		$this->_field = field::createDummyField();
...
class map_orm {
	...
	function setKeyValues($m,$fds,$keyNum) {
		map_orm_setKeyValues($this->_cPtr,$m,$fds,$keyNum);
	}

	function writeMap($m,$fds,$optipn) {
		map_orm_writeMap($this->_cPtr,$m,$fds,$optipn);
	}

	function readAuntoincValue($m,$fds,$optipn) {
		map_orm_readAuntoincValue($this->_cPtr,$m,$fds,$optipn);
	}
}
...
class groupQuery {
	...
	function keyField($name,$name1=null,$name2=null,$name3=null,$name4=null,$name5=null,$name6=null,$name7=null,$name8=null,$name9=null,$name10=null) {
		$r=groupQuery_keyField($this->_cPtr,$name,$name1,$name2,$name3,$name4,$name5,$name6,$name7,$name8,$name9,$name10);
		if (!is_resource($r)) return $r;
		switch (get_resource_type($r)) {
	...
	function resultField($name) {
		$r=groupQuery_resultField($this->_cPtr,$name);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
			return new groupQuery($r);
